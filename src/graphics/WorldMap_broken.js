...
WorldMap.prototype.openRouteConfigPanel = function() {
    ...
    return false;
        ...
        e.updateConfigEstimates = function() {
        // Imposta stato configurazione  var analysisAccuracy = this.routeConfigState.analysisLevel === 'improved' ? '±10%' : '±30%';
        this.routeConfigState.isOpen = true;    var cargoAccuracy = this.routeConfigState.analysisLevel === 'improved' ? '±8%' : '±25%';
        ...
        this.updateRouteConfigDisplay();Content = passengers;
        document.getElementById('config-cargo').textContent = cargo;
        console.log('✅ Vista configurazione rotta attivata');
        return true;
    }acy');
    if (accuracySpans[0]) accuracySpans[0].textContent = '(' + analysisAccuracy + ')';
    console.error('❌ Elementi vista non trovati');ccuracySpans[1].textContent = '(' + cargoAccuracy + ')';
    return false;
};

WorldMap.prototype.closeRouteConfigPanel = function() {    this.updateConfigCosts();
    console.log('⚙️ Chiusura vista configurazione rotta...');
    
    // Torna alla vista mappa
    var mapView = document.getElementById('map-view');Map.prototype.updateConfigCosts = function() {
    var configView = document.getElementById('route-config-view');  var routeType = this.routeConfigState.routeType;
        var distance = this.routeConfigState.distance;
    if (mapView && configView) {
        configView.classList.remove('active');
        mapView.classList.add('active');
        if (routeType === 'passenger') {
        // Reset stato configurazioneouteConfigState.estimatedPassengers * 120; // €120 per passeggero medio
        this.routeConfigState.isOpen = false;=== 'cargo') {
        figState.estimatedCargo * 800; // €800 per tonnellata
        // Reset analisi di mercato
        this.routeConfigState.marketAnalysis = false;gState.estimatedPassengers * 120) + 
        this.updateMarketAnalysisDisplay();
        
        // Reset tipo rotta a passeggeri
        this.selectRouteType('passenger');
        
        console.log('✅ Vista mappa ripristinata');// Calcola costi operativi (più alti per distanze maggiori)
        return true;2);
    }var flightsPerMonth = 30; // 1 volo al giorno
    
    console.error('❌ Elementi vista non trovati');
    return false;yCost;
};
// Aggiorna display
WorldMap.prototype.updateConfigEstimates = function() {ent = costPerFlight.toLocaleString();
    var analysisAccuracy = this.routeConfigState.analysisLevel === 'improved' ? '±10%' : '±30%';document.getElementById('config-monthly-revenue').textContent = monthlyRevenue.toLocaleString();
    var cargoAccuracy = this.routeConfigState.analysisLevel === 'improved' ? '±8%' : '±25%';ById('monthly-operating-cost').textContent = monthlyCost.toLocaleString();
    ();
    var passengers = this.routeConfigState.estimatedPassengers;
    var cargo = this.routeConfigState.estimatedCargo;
    
    // Aggiorna display stimeif (profitElement) {
    document.getElementById('config-passengers').textContent = passengers;le.color = estimatedProfit > 0 ? '#27ae60' : '#e74c3c';
    document.getElementById('config-cargo').textContent = cargo;
