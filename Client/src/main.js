...
document.addEventListener('DOMContentLoaded', function() {
    console.debug('🛫 Air Tycoon 2 Clone - Avvio in corso...');
    ...
    function loadCoreDataAndStartGame(companyId) {
        ...
        fetch('/api/game/aircraft-data').then(r => r.json())
            .then(aircraftData => {
                ...
                console.debug('✅ AircraftData caricato via API');
    ...
    console.debug('✅ Leaflet caricato');
...
function initializeGame(companyId) {
    ...
    console.debug('✅ Tutte le classi e i dati fondamentali sono caricati');
    ...
    console.debug('🎮 Creazione istanza Game...');
        ...
        window.game = game;
    console.debug('✅ Game creato');
        window.addEventListener('error', handleGlobalError);
        window.addEventListener('unhandledrejection', handleUnhandledRejection);
        window.addEventListener('resize', handleWindowResize);
        document.addEventListener('visibilitychange', handleVisibilityChange);
    console.debug('✅ Event listeners configurati');
...
function handleVisibilityChange() {
    ...
    console.debug('⏸️ Gioco in pausa (pagina nascosta)');
    } else {
        // Pagina visibile - riprendi il gioco
        if (game.start) {
            game.start();
        }
    console.debug('▶️ Gioco ripreso (pagina visibile)');
    }
}
