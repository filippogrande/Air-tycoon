...
// Expose for CommonJS and browser globals
try {
...
// ES module compatibility (some dev servers may serve this as ESM)
try {
...
WorldMap.prototype.loadAirports = function() {
    ...
    // 1. Prima carica gli hub della compagnia
...
WorldMap.prototype.setupRouteCreationEvents = function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (purchaseBtn.classList && purchaseBtn.classList.contains('buy-aircraft-btn')) {
            ...
            // Invece di cambiare direttamente la tab e sostituire il contenuto,
            // dispatchiamo un evento che aprirà l'interfaccia di acquisto dentro la tab Flotta.
            try {
                const evt = new CustomEvent('open-purchase', { detail: { origin: 'route-creator' } });
                document.dispatchEvent(evt);
            } catch (err) {
                console.warn('Errore dispatch open-purchase:', err);
                // Fallback: prova a cambiare tab come prima
                if (self.game && self.game.uiManager && self.game.uiManager.switchTab) {
            ...
            if (self.game && self.game.uiManager) {
                self.game.uiManager.showNotification('Aperto catalogo Flotta', 'info');
            }
        }
