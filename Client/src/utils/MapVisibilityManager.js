...
console.debug('📂 Caricamento MapVisibilityManager.js...');
...
var MapVisibilityManager = {
	...
	console.debug('🔍 Aggiornamento visibilità aeroporti intelligente, zoom:', zoom);
		...
		if (zoom >= 10) {
			...
			console.debug('✅ [FORZATO] Tutti gli aeroporti visibili:', visibleCount, '/', Object.keys(airportMarkers).length);
	...
	console.debug('✅ Aeroporti visibili:', visibleCount, '/', Object.keys(airportMarkers).length, 
	...
	console.debug('🏷️ Etichette città per zoom', zoom + ':',
				   'Large ≥5:', zoom >= 5,
				   'Medium ≥6:', zoom >= 6, 
				   'Small ≥8:', zoom >= 8);
	},
	...
	calculateAirportRating: function(airport) {
		...
		if (Math.random() < 0.1) { // 10% delle volte
			console.debug('🧮 Rating calcolato per', airport.code + ':', 
...
console.debug('✅ MapVisibilityManager caricato');
